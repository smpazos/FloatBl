...
System { *contains the netlist
  ...
  *-Lumped elements:
  ...
  Resistor_pset Rdrain ( out dr ){ resistance = 10000 }
  nmos_bsim3 mosnbulk ( subs  gb  0  0 ) { l=10u w=100u }
  Capacitor_pset cout ( subs 0 ) { capacitance = 2000e-12 }
  Vsource_pset vgb (gb 0) { dc = 0.8 } 
...
#-- simlations can be done in transient in a similar manner (commented out)  
   Transient (
      InitialTime=0 FinalTime=0.7
      InitialStep=1e-6 Increment=2 Decrement=3
      MaxStep=0.01 MinStep=1e-13
    ){ 
      Coupled { nmos1.poisson nmos1.electron nmos1.hole nmos1.contact circuit }
      *- Plot(FilePrefix= "n@node@" NoOverWrite Time = (Range = (0 0.7)  Intervals = 200)) 
  }
